...
public class BinarySearchTree {
	...
	public boolean isHeightBalanced() {
		HeightWrapper output = this.root.isHeightBalanced();
	...
	public class BinaryNode {
		...
		public HeightWrapper isHeightBalanced() {
			if (this == NULL_NODE) {
				return new HeightWrapper(true, -1);
			}
			HeightWrapper tempLeft = this.left.isHeightBalanced();
			HeightWrapper tempRight = this.right.isHeightBalanced();
			Boolean thisBalance = Math.abs(tempLeft.height - tempRight.height) <= 1;
			return new HeightWrapper(tempLeft.balanced && tempRight.balanced && thisBalance,
					1 + Math.max(tempLeft.height, tempRight.height));
		}
	}
	...
	class HeightWrapper {
		...
		public HeightWrapper(boolean balanced, int height) {
